...
@ServiceHandler(id="taiji.io/kyc/update/1.0.0")
public class UpdateKyc implements Handler {
    ...
    public ByteBuffer handle(HttpServerExchange exchange, Object input)  {
        ...
        // get the KYC from the server by address
        String currency = null;
        Result<String> kycResult = TaijiClient.getKycByAddress(address);
        if(kycResult.isSuccess()) {
            Map<String, Object> kycMap = JsonMapper.string2Map(kycResult.getResult());
            currency = (String)kycMap.get("currency");
        } else {
            NioUtils.toByteBuffer(getStatus(exchange, kycResult.getError()));
        }
        ...
        KycUpdatedEvent kycUpdatedEvent = KycUpdatedEvent.newBuilder()
                .setEventId(eventId)
                .setName(name)
                .setTags(tags)
        ...
        byte[] bytes = serializer.serialize(kycUpdatedEvent);
